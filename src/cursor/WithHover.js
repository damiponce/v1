...
export default (Component, type, config) =>
    ({ passThroughRef, ...props }) => {
        ...
        if (hovering && selectedElement.el && selectedElement.type == 'block') {
            const amount = selectedElement.config.hoverOffset
                ? selectedElement.config.hoverOffset
                : 2;
            ...
            const xMove =
                ((relativePos.x - xMid) / selectedElement.el.offsetHeight) *
                amount;
            const yMove =
                ((relativePos.y - yMid) / selectedElement.el.offsetHeight) *
                amount;
