...
export default function Viewer({
   ...
   return (
      <div className={styles.container}>
         ...
         {currIndex > 0 ? (
            <div
               className={styles.left + ' ' + styles.button + mobileButtons}
               onClick={() => (
                  setCurrPic(''),
         ...
         {currIndex + 1 < picArray.length ? (
            <div
               className={styles.right + ' ' + styles.button + mobileButtons}
               onClick={() => (
                  setCurrPic(''),
                  setCurrIndex(
                     currIndex + 1 < picArray.length ? currIndex + 1 : currIndex
                  ),
                  setCurrPic(
                     picArray[
                        currIndex + 1 < picArray.length
               ...
               style={
                  picArray[currIndex].ratio > 1
                     ? {
                          position: 'relative',
                          width: '100vw',
                          height: window.innerWidth / picArray[currIndex].ratio,
                          //  position: 'absolute',
                          //  top: `${state.y * 100}%`,
                          //  left: `${state.x * 100}%`,
                          //  transform: `scale(${
                     ...
                     : {
                          position: 'relative',
                          height: '100vh',
                          width: window.innerHeight * picArray[currIndex].ratio,
                          //  position: 'absolute',
                          //  top: `${state.y * 100}%`,
                          //  left: `${state.x * 100}%`,
